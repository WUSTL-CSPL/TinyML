...
__STATIC_FORCEINLINE uint32_t my1__SXTB16_RORn(uint32_t op1, uint32_t rotate)
{
  uint32_t result;
    ...
    for (int32_t rhs_rows_idx = 0; rhs_rows_idx <= (rhs_rows - 2); rhs_rows_idx += 2)
    {
        ...
        while (lhs_rows_idx)
        {
            ...
            for (; rhs_cols_idx <= (rhs_cols - 16); rhs_cols_idx += 16)
            {
                ...
                val1 = my1__SXTB16_RORn(val1, 8);
                val0 = my1__SXTB16_RORn(val0, 8);

                // 4 x MAC res00, res01
                res00 = __SMLAD(val3, val2, res00);
                val5 = __SXTB16(val4);
                res00 = __SMLAD(val0, val1, res00);
                val4 = my1__SXTB16_RORn(val4, 8);
